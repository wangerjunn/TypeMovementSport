...
#import "Course_yiBingWorldView.h"
...
@interface Course_yiBingWorldView () <UITableViewDelegate,UITableViewDataSource> {
    UITableView *yiBingTable;//实操table
...
@implementation Course_yiBingWorldView
...
- (void)createUI {
    yiBingTable = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, self.width, self.height) style:UITableViewStyleGrouped];
    yiBingTable.delegate = self;
    yiBingTable.dataSource = self;
    yiBingTable.separatorStyle = UITableViewCellSeparatorStyleNone;
    if (@available(iOS 11.0,*)) {
        yiBingTable.estimatedRowHeight = 0;
        yiBingTable.estimatedSectionHeaderHeight = 0;
        yiBingTable.estimatedSectionFooterHeight = 0;
        yiBingTable.contentInsetAdjustmentBehavior = UIScrollViewContentInsetAdjustmentNever;
    }
    
    yiBingTable.rowHeight = FIT_SCREEN_HEIGHT(105);
    [self addSubview:yiBingTable];
    
    TO_WEAK(self, weakSelf);
    yiBingTable.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
...
- (EmptyView *)emptyView {
    if (!_emptyView) {
        _emptyView = [[EmptyView alloc] initEmptyViewByFrame:yiBingTable.bounds placeholderImage:nil placeholderText:nil];
        [yiBingTable addSubview:_emptyView];
    }
...
- (void)getVideoDataList {
    ...
    [WebRequest PostWithUrlString:kApiClassFindAllByParent parms:para viewControll:nil success:^(NSDictionary *dict, NSString *remindMsg) {
        ...
        if ([remindMsg integerValue] == 999) {
            ...
            [strongSelf->yiBingTable reloadData];
        }else {
        ...
        [strongSelf->yiBingTable.mj_header endRefreshing];
